...
def get_api_indicators_incomes():
    df = pd.read_json(URL_API + '/indicators_incomes')
    d = df['Name_indicator']
    return d
...
def get_api_incomes_ine(year):
    url = URL_API + f'/incomes_ine/{year}'
    df = pd.read_json(url)
    return df
...
def get_api_incomes_aeat(year):
    url = URL_API + f'/incomes_aeat/{year}'
    df = pd.read_json(url)
    return df

# get years incomes AEAT
def get_api_incomes_aeat_years():
    url = URL_API + f'/incomes_aeat/years'
    df = pd.read_json(url)
    if df.empty:
        return 0, 1
    else:
...
def get_api_incomes_ine_years():
    url = URL_API + f'/incomes_ine/years'
    df = pd.read_json(url)
    if df.empty:
        return 0, 1
    else:
...
def build_graph_incomes_ine(df):
    ...
    fig = make_subplots(rows=1, 
                        cols=2)
    fig.add_trace(go.Bar(x=data.nlargest(5, 'Total')['City'], y=data.nlargest(5, 'Total')['Total']),
                  row=1, col=1
    )
    fig.add_trace(go.Bar(x=data.nsmallest(5, 'Total')['City'], y=data.nsmallest(5, 'Total')['Total']),
                  row=1, col=2
    )
    fig.update_layout(height=400, 
                      width=800, 
                      showlegend=False, 
                      title_text='Top and bottom cities (population >1000): Average net income per person (EUR)')
    list_fig.append(fig)
    ...
    fig.update_layout(height=400, 
                      ...
                      legend_title_text='Indicator incomes (EUR)')
    list_fig.append(fig)
...
def get_url_tableau(type):
    if type == 'incomes_ine':
        # year 2020
        url = 'https://public.tableau.com/app/profile/elvira8700/viz/IncomesINE/IncomesINE'
    else:
        url = ''
    return url
...
def play_population_ine():
    ...
    st.write('Info by [Instituto Nacional de Estadística](https://www.ine.es/)')
    ...
    with st.spinner('Loading...'):
        ...
        df_pop_dtype = get_data_type(df_pop, 'population_ine')
        ...
        # plot
        plots_pop = get_plot('population_ine', df_pop)
        for gr_pop in plots_pop:
            st.plotly_chart(gr_pop, use_container_width=True)
        # url tableau
        url_tab_p = get_url_tableau('population_ine')
        st.write(f'For more visualizations, you can see [Tableau Dashboard]({url_tab_p})')
        # component tableau
        #html_tab_p = get_html_tableau('population_ine')
        #components.html(html_tab_p)


# Income INE collect
def play_incomes_ine():
    # header
    st.header('Incomes in Spain')
    st.text('Get distribution of incomes by city')
    st.write('Info by [Instituto Nacional de Estadística](https://www.ine.es/)')
    ...
    with st.spinner('Loading...'):
        ...
        df_incomes_ine_dtype = get_data_type(df_incomes_ine, 'incomes_ine')
        ...
        st.subheader('Data overview')
        # plot
        plots_in_i = get_plot('incomes_ine', df_incomes_ine)
        for gr_in_i in plots_in_i:
            st.plotly_chart(gr_in_i, use_container_width=True)
        # url tableau
        url_tab_i = get_url_tableau('incomes_ine')
        st.write(f'For more visualizations, you can see [Tableau Dashboard]({url_tab_i})')
        # component tableau
        #html_tab_i = get_html_tableau('incomes_ine')
        #components.html(html_tab_i)
    
# Income AEAT collect
def play_incomes_aeat():
    # header
    st.header('Incomes in Spain')
    st.text('Get distribution of incomes by city')
    st.write('Info by [Agencia Estatal de Administración Tributaria](https://sede.agenciatributaria.gob.es/)')
    ...
    with st.spinner('Loading...'):
        ...
        df_incomes_aeat_dtype = get_data_type(df_incomes_aeat, 'incomes_aeat')
        ...
        st.subheader('Data overview')
        # plot
        plots_in_a = get_plot('incomes_aeat', df_incomes_aeat)
        for gr_in_a in plots_in_a:
            st.plotly_chart(gr_in_a, use_container_width=True)
        # url tableau
        url_tab_ia = get_url_tableau('incomes_aeat')
        st.write(f'For more visualizations, you can see [Tableau Dashboard]({url_tab_ia})')
        # component tableau
        #html_tab_ia = get_html_tableau('incomes_aeat')
...
tab_pop_ine, tab_incomes_ine, tab_incomes_aeat = st.tabs(['Population INE', 'Incomes INE', 'Incomes AEAT'])

with tab_pop_ine:
   play_population_ine()

with tab_incomes_ine:
   play_incomes_ine()

with tab_incomes_aeat:
   play_incomes_aeat()
