...
def load_incomes(year):
    ...
    name_table = year + '_' + INCOME_TABLE
    save_incomes(df_incomes, name_table)

# check integrity incomes
def check_integrity_income(year):
    table_name = year + '_' + INCOME_TABLE
    msg = f'Integrity incomes {table_name}: '
    city_ok = db_int.integrity_city(table_name)
    indi_ok = db_int.integrity_indicator_income(table_name)
